...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/slack-backend/src/assistant/actions/feedback-handler.ts"
      ...
      via: "trackFeedback call with 'liked' or 'disliked' action"
      pattern: "trackFeedback"
    - from: "apps/slack-backend/src/assistant/actions/send-from-assistant.ts"
      to: "apps/slack-backend/src/assistant/streaming.ts"
      via: "reads viewingChannelId and viewingThreadTs from button value JSON"
      pattern: "viewingThreadTs"
---
...
<task type="auto">
  <name>Task 1: Extend feedback tracker and create feedback handler and assistant action buttons</name>
  <files>
    apps/slack-backend/src/services/feedback-tracker.ts
    apps/slack-backend/src/assistant/actions/feedback-handler.ts
    apps/slack-backend/src/assistant/actions/send-from-assistant.ts
  ...
  <action>
0. Update `apps/slack-backend/src/services/feedback-tracker.ts`:
   - Extend the `TrackFeedbackParams.action` type to include quality rating values:
     ```typescript
     action: 'accepted' | 'refined' | 'dismissed' | 'sent' | 'liked' | 'disliked';
     ```
   - This distinguishes quality ratings (liked/disliked from thumbs up/down) from action types (accepted/refined/dismissed/sent). The DB column is `text` so no migration needed -- this is a TypeScript-only change.
   - Do NOT modify any existing function signatures or behavior. Only widen the union type.
...
1. Create `apps/slack-backend/src/assistant/actions/feedback-handler.ts`:
   ...
   - Map feedback to the correct action values: `feedback === 'positive' ? 'liked' : 'disliked'`
   - Call the existing `trackFeedback` service from `../../services/feedback-tracker.js` to store the feedback:
     ...
     await trackFeedback({
       ...
       userId,
       workspaceId,  // extract from body.team?.id via getWorkspaceId
     });
     ```
   - Note: `'liked'` and `'disliked'` are quality rating values, distinct from the existing action types (`'accepted'`, `'dismissed'`). Thumbs up does NOT mean the user accepted/used the suggestion -- it means they rated it positively.
...
2. Create `apps/slack-backend/src/assistant/actions/send-from-assistant.ts`:
   ...
   - The `channelId` is the VIEWING channel (where to post), NOT the assistant thread channel. The `threadTs` is the `viewingThreadTs` from StreamOptions (may be undefined for channel-level messages).
...
4. Create `apps/slack-backend/src/assistant/actions/dismiss-from-assistant.ts`:
   ...
   - Track dismissal via existing feedback tracker (using action `'dismissed'`)
   - Respond: `await say({ text: "Suggestion dismissed. Send me another message if you need help." })`
  </action>
  <verify>
All five files exist. `grep "'liked'" apps/slack-backend/src/services/feedback-tracker.ts` confirms the new action type. `grep "'liked'" apps/slack-backend/src/assistant/actions/feedback-handler.ts` confirms correct mapping. `npm run build --workspace=@slack-speak/slack-backend` compiles without errors.
  </verify>
  <done>
Feedback tracker extended with 'liked' and 'disliked' action types for quality ratings. Four action handlers created: ai_feedback (thumbs up/down mapped to liked/disliked), assistant_send_suggestion (Send as Me from panel using viewingChannelId + viewingThreadTs), assistant_refine_suggestion (prompts for refinement), assistant_dismiss_suggestion (dismisses with tracking).
  </done>
</task>
...
2. Update `apps/slack-backend/src/assistant/streaming.ts`:
   ...
   - Actually, the action buttons should be sent as a SEPARATE message after the stream stops (the stream contains the suggestion text; actions go below it). After `streamer.stop()`, post a follow-up message in the thread with action buttons:
     ...
     ```
   - Note: `viewingChannelId` and `viewingThreadTs` are both available from the `StreamOptions` interface (defined in Plan 15-04 Task 2 with both fields). The `viewingThreadTs` may be undefined if the user is viewing a channel rather than a specific thread -- this is fine, the Send as Me handler posts to channel level when threadTs is absent.
...
4. Export `registerAssistantActions` from the handlers index if needed for consistency, OR import it directly in app.ts (prefer direct import in app.ts since it's assistant-specific and the handlers/index.ts is for the existing handler tree).
  ...
  <verify>
`npm run build --workspace=@slack-speak/slack-backend` compiles. `grep 'registerAssistantActions' apps/slack-backend/src/app.ts` returns a match. `grep 'assistant_send_suggestion' apps/slack-backend/src/assistant/streaming.ts` shows action buttons are included in streaming output. `grep 'viewingThreadTs' apps/slack-backend/src/assistant/streaming.ts` confirms thread timestamp is passed to Send as Me button value.
  </verify>
  <done>
Action buttons (Send as Me, Refine, Dismiss) appear after every streamed suggestion in the assistant panel. Send as Me button value includes viewingChannelId and viewingThreadTs from StreamOptions. Feedback buttons (thumbs up/down) appear at the end of the streamed text. All action handlers registered with the app. Users can interact with suggestions directly from the assistant panel.
  </done>
</task>
...
5. `grep "'liked'" apps/slack-backend/src/services/feedback-tracker.ts` -- quality rating action types added
6. `grep 'viewingThreadTs' apps/slack-backend/src/assistant/streaming.ts` -- thread ts wired to action buttons
</verification>

<success_criteria>
- Feedback buttons (thumbs up/down) track quality via 'liked'/'disliked' action types (not 'accepted'/'dismissed')
- Send as Me posts suggestion in the viewing channel + thread using user's token
...
- viewingThreadTs flows from StreamOptions through to Send as Me button value
</success_criteria>
