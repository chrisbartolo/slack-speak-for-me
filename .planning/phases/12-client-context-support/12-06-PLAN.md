...
must_haves:
  truths:
    - "When a user asks for a suggestion in a conversation with a client contact, the suggestion includes client-specific context (company name, services, contract)"
    - "Suggestions for client conversations follow the organization's brand voice guidelines"
    - "When a client message shows frustration or tension, the suggested response uses de-escalation techniques (empathy, acknowledgment, next steps)"
    - "Suggestions reference relevant product/service documentation from the knowledge base when applicable"
    - "If any client context service fails, the user still receives a standard suggestion without errors"
  artifacts:
...
- The existing flow for non-client conversations remains UNCHANGED
  ...
  <verify>Run `npx tsc --noEmit` in apps/slack-backend. Then verify each service function is actually called in ai.ts:
- `grep 'getClientContactBySlackUserId' apps/slack-backend/src/services/ai.ts` (must match import AND call)
- `grep 'getBrandVoiceContext' apps/slack-backend/src/services/ai.ts` (must match import AND call)
- `grep 'analyzeSentiment' apps/slack-backend/src/services/ai.ts` (must match import AND call)
- `grep 'searchKnowledgeBase' apps/slack-backend/src/services/ai.ts` (must match import AND call)
- `grep 'client_context' apps/slack-backend/src/services/ai.ts` (XML tag in prompt)
- `grep 'de_escalation_mode' apps/slack-backend/src/services/ai.ts` (XML tag in prompt)
- `grep 'knowledge_base' apps/slack-backend/src/services/ai.ts` (XML tag in prompt)
- `grep -c '\.catch(' apps/slack-backend/src/services/ai.ts` (should show at least 4 .catch() calls for the 4 new external calls)
Each grep must return at least one match. If any grep returns no match, the integration is incomplete.</verify>
