...
**Embedding approach:** Use Anthropic's Claude to generate a text summary/description, then use the existing pgvector pattern from historyAnalyzer.ts for the embedding. Check how historyAnalyzer.ts handles embeddings -- it likely uses a helper. If not, use raw SQL with pgvector.

Actually, looking at the research and existing codebase: the project already uses pgvector from Phase 3 (messageEmbeddings table). Follow the SAME embedding approach used in historyAnalyzer.ts. If it uses OpenAI embeddings, use the same model. If it uses a different approach, mirror it.

**Functions to create:**

1. `embedText(text: string): Promise<number[]>` - Generate embedding vector for text. Use the same embedding model/approach as historyAnalyzer.ts. If no embedding model is currently configured, use Anthropic's message API to create a summary and then generate a simple hash-based embedding as fallback, OR add the OpenAI embedding SDK. Check historyAnalyzer.ts first.
...
<task type="auto">
  <name>Task 2: Create background indexer job and admin UI</name>
  <files>apps/slack-backend/src/jobs/knowledge-base-indexer.ts, apps/slack-backend/src/jobs/types.ts, apps/slack-backend/src/jobs/queues.ts, apps/slack-backend/src/jobs/workers.ts, apps/web-portal/app/api/admin/knowledge-base/route.ts, apps/web-portal/app/api/admin/knowledge-base/[id]/route.ts, apps/web-portal/app/admin/knowledge-base/page.tsx</files>
  <action>
**Background Job:**
...
Server component page following admin pattern:
...
- Show "Indexing..." indicator while document is being processed (embedding = empty means pending)
  ...
  <verify>Run `npx tsc --noEmit` in both workspaces. Check jobs/workers.ts includes KB worker in startWorkers/stopWorkers.</verify>
  <done>Knowledge base indexer job processes document embeddings asynchronously. Admin page provides full CRUD for knowledge base documents with category management.</done>
</task>
