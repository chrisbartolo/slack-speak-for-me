...
<task type="auto">
  <name>Task 2: Create API routes for client profile CRUD</name>
  <files>apps/web-portal/app/api/admin/clients/route.ts, apps/web-portal/app/api/admin/clients/[id]/route.ts, apps/web-portal/app/api/admin/clients/[id]/contacts/route.ts</files>
  <action>
Create three API route files for client management.
...
- DELETE: Remove contact by contactId in body
  ...
  <verify>Run `npx tsc --noEmit` in apps/web-portal. Grep route.ts files for GET, POST, PUT, DELETE exports. Use `curl -X GET http://localhost:3000/api/admin/clients` to verify 200 response (if dev server running).</verify>
  <done>API routes handle create, read, update, delete for client profiles and contacts. All endpoints validate org ownership via requireAdmin().</done>
</task>

<task type="auto">
  <name>Task 3: Create admin client profiles page</name>
  <files>apps/web-portal/app/admin/clients/page.tsx</files>
  <action>
Create the admin client management page.
...
Form submits via fetch to /api/admin/clients (POST for create, PUT for edit). Delete triggers fetch to /api/admin/clients/[id] (DELETE). Contacts managed via /api/admin/clients/[id]/contacts.
  </action>
  <verify>Run `npx tsc --noEmit` in apps/web-portal. Grep page.tsx for 'requireAdmin' and 'clientProfiles'. Navigate to /admin/clients page (visual check if dev server running).</verify>
  <done>Admin page shows client profiles with status badges and management actions. Create/edit/delete operations work via API routes from Task 2.</done>
</task>
...
- End-to-end: Admin creates a client profile via the page, adds a contact, and getClientContactBySlackUserId returns the contact for that Slack user
</success_criteria>
