...
**First: Verify app.ts exports the Express app for supertest:**

Check `src/app.ts` for export. The Bolt app uses ExpressReceiver, so:
- `export const app = new App(...)` exists - this is the Bolt app
- For supertest, we need the underlying Express app: `receiver.app`

Add export if not present:
```typescript
// At end of app.ts
export const expressApp = receiver.app;  // For testing with supertest
```

**Handling async job processing in E2E tests:**

In E2E tests, we want synchronous execution flow. Use vi.mock to make queueAIResponse call worker logic directly:

```typescript
import { vi, describe, it, expect, beforeAll, afterAll, beforeEach } from 'vitest';
import { processAIResponseJob } from '../../src/jobs/workers';

// Mock the queue to call processor synchronously
vi.mock('../../src/jobs/queues', async () => {
  const actual = await vi.importActual('../../src/jobs/queues');
  return {
    ...actual,
    queueAIResponse: vi.fn(async (jobData) => {
      // Call worker processor directly instead of adding to queue
      const mockJob = { id: 'e2e_test_job', data: jobData, attemptsMade: 0 };
      return processAIResponseJob(mockJob as any);
    }),
  };
});
```
...
// Mock queue to process synchronously (see above)
vi.mock('../../src/jobs/queues', async () => {
  const actual = await vi.importActual('../../src/jobs/queues');
  return {
    ...actual,
    queueAIResponse: vi.fn(async (jobData) => {
      const mockJob = { id: 'e2e_test_job', data: jobData, attemptsMade: 0, updateProgress: vi.fn(), log: vi.fn() };
      return processAIResponseJob(mockJob as any);
    }),
  };
});
...
describe('App Mention E2E', () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(schema.workspaces).values({ teamId: 'T123', name: 'Test Workspace' });
  ...
  it('should process app_mention and deliver suggestion', async () => {
    ...
    // Set env var: process.env.SLACK_SKIP_SIGNATURE_VERIFICATION = 'true'
    ...
    // With synchronous processing (via mock), ephemeral should be sent immediately
  ...
  it('should include context in AI request', async () => {
    ...
    // Verify AI received the message context
    expect(aiRequestBody).toBeTruthy();
    expect(aiRequestBody.messages).toBeDefined();
  });
});
```

**Challenges addressed:**
1. Bolt signature verification - set `SLACK_SKIP_SIGNATURE_VERIFICATION=true` in test env
2. Async job processing - mocked queue calls processor synchronously
3. Database seeding - insert valid workspace/installation for lookup
4. App export - expressApp exported from app.ts for supertest
  </action>
  <verify>
...
// Mock queue for synchronous processing (same as app-mention.e2e.test.ts)
vi.mock('../../src/jobs/queues', async () => {
  const actual = await vi.importActual('../../src/jobs/queues');
  const { processAIResponseJob } = await import('../../src/jobs/workers');
  return {
    ...actual,
    queueAIResponse: vi.fn(async (jobData) => {
      const mockJob = { id: 'e2e_test_job', data: jobData, attemptsMade: 0, updateProgress: vi.fn(), log: vi.fn() };
...
- Queue mocked to call processor synchronously (no async delay)
- expressApp exported from app.ts for supertest compatibility
