...
export function GoogleConnectionCard({ isConnected, spreadsheetName }: Props) {
  ...
  const handleConnect = () => {
    ...
    // Get workspaceId and userId from session - we'll pass via server action
...
Note: The handleConnect needs workspaceId/userId. For MVP, we can either:
1. Use a server action to get the auth URL (includes session data)
2. Use Next.js middleware to inject session into a cookie readable client-side

Use approach 1 - update to use server action that returns redirect URL.
  </action>
  <verify>
`npm run build --workspace=web-portal` compiles.
`npm run build --workspace=slack-backend` compiles.
