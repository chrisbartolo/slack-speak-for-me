...
must_haves:
  truths:
    - "Admin can view current team health score, NPS breakdown, and survey response rates from a single API"
    - "Admin can download satisfaction data as CSV for external analysis"
    - "Admin can compare baseline communication quality against current scores to measure improvement"
    - "Dashboard queries return results within acceptable load time using React cache"
  artifacts:
