...
<task type="auto">
  ...
  <action>
NOTE: This task touches 9 files but each modification is small (1-10 lines per file for wiring: types, queues, exports, imports). The bulk of the logic is in the action handler file itself.
...
- Extract surveyId from `action.value` (the button's value field set to surveyId in buildSurveyBlocks)
- Extract rating from the radio_buttons state using this exact path: `body.state.values['satisfaction_survey_' + surveyId]['satisfaction_rating'].selected_option.value` -- parse the value to int with `parseInt(..., 10)`. The surveyId is obtained from action.value above, which provides the key to look up the correct block_id in state.values.
- Extract feedback text from block state: `body.state.values['satisfaction_feedback']['feedback_text'].value` -- this may be undefined/null if user skipped the optional input
