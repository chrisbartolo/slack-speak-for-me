...
<task type="auto">
  ...
  <action>
    ...
    4. Export usageRecords and usageEvents from `apps/web-portal/lib/db/index.ts` if not already exported (check first).
...
<task type="auto">
  ...
  <action>
    1. The PLAN_LIMITS in usage-enforcement.ts is manually duplicated from plans.config.ts. Update it to match the current plan IDs in plans.config.ts exactly:
       ...
       Remove `individual_starter`, `individual_pro`, `team_starter`, `team_pro` entries since the plan IDs in plans.config.ts are `starter`, `pro`, `team`, `business`.
  ...
  <verify>
    ...
    - Verify PLAN_LIMITS keys match plans.config.ts plan IDs (starter, pro, team, business, free)
  </verify>
  <done>
    PLAN_LIMITS aligned with plans.config.ts. Race condition fixed with atomic SQL increment. Warning levels computed from thresholds. getUsageStatus returns warningLevel and overageRate.
  </done>
</task>
