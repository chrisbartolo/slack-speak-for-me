...
must_haves:
  truths:
    ...
    - "Stripe customer ID resolved via dual-path: individual subscription first, then organization fallback"
  artifacts:
...
<task type="auto">
    ...
    1. Create `apps/slack-backend/src/services/stripe-meter-reporter.ts`:
       ...
       - Function `reportUnreportedUsageBatch()`:
         ...
         - For each unreported event:
           a. Look up the user's Stripe customer ID using dual-path resolution:
              - **Path 1 (Individual):** Query `userSubscriptions` by email to get `stripeCustomerId`. If `stripeCustomerId` is non-null, use it.
              - **Path 2 (Organization):** If Path 1 yields no stripeCustomerId, join `users` (by email) -> `workspaces` (by workspaceId) -> check `workspaces.organizationId` -> query `organizations` table to get `organizations.stripeCustomerId`.
              - Field names: `userSubscriptions.stripeCustomerId`, `organizations.stripeCustomerId`, `workspaces.organizationId`
           b. If no Stripe customer found via either path, skip (user may be on free tier)
           c. Call `reportUsageToStripe()` with the resolved customer ID and event details
  ...
  <verify>
    ...
    - Grep for `STRIPE_METER_EVENT_NAME` in stripe-meter-reporter.ts to confirm meter event name is configurable via env var
    - Grep for `userSubscriptions.*stripeCustomerId` in stripe-meter-reporter.ts to confirm individual path lookup
    - Grep for `organizations.*stripeCustomerId` in stripe-meter-reporter.ts to confirm org path lookup
  </verify>
  <done>
