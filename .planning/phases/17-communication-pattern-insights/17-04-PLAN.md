...
must_haves:
  truths:
    ...
    - "getClientInsights returns per-client topic breakdown and sentiment summary when client profiles exist"
  ...
  artifacts:
    - path: "apps/web-portal/lib/admin/communication-insights.ts"
      ...
      exports: ["getTopicOverview", "getTopicTrend", "getSentimentTrend", "getChannelHotspots", "getWeekOverWeek", "getEscalationSummary", "getClientInsights"]
  key_links:
    ...
    - from: "apps/web-portal/lib/admin/communication-insights.ts"
      to: "packages/database/src/schema.ts"
      via: "getClientInsights joins topicClassifications -> clientContacts -> clientProfiles"
      pattern: "clientContacts|clientProfiles"
---
...
4. `getClientInsights` — cached function:
   - Parameters: organizationId, workspaceId, days (default 30)
   - Calls `requireAdmin()`
   - First, check if org has any client profiles:
     ```sql
     SELECT COUNT(*) as count FROM client_profiles WHERE organization_id = $orgId
     ```
   - If count is 0, return empty array immediately (no client profiles configured)
   - If client profiles exist, query topic_classifications joined through client_contacts to client_profiles:
     ```sql
     SELECT
     ...
     INNER JOIN client_contacts cc
       ON cc.slack_user_id = tc.user_id
       AND cc.workspace_id = tc.workspace_id
     INNER JOIN client_profiles cp
       ON cp.id = cc.client_profile_id
       AND cp.organization_id = tc.organization_id
     WHERE tc.organization_id = $orgId
       AND tc.created_at >= $startDate
     GROUP BY cp.id, cp.company_name, tc.topic, tc.sentiment->>'tone'
     ORDER BY cp.company_name ASC
     ```
   - Aggregate results in JavaScript per client:
     - `totalMessages`: sum of all topic counts for that client
     - `topTopic`: topic with the highest count
     - `topicBreakdown`: Record<string, number> of topic -> count
     - `complaintRate`: (complaint count / totalMessages) * 100, to 1 decimal
     - `escalationCount`: count of 'escalation' topic entries
     - `dominantSentiment`: tone with the highest count
   - Returns ClientInsight[]

Use the same result parsing pattern from Task 1 for all queries.
  </action>
  <verify>Run `npx tsc --noEmit` in apps/web-portal. Verify getChannelHotspots, getWeekOverWeek, getEscalationSummary, getClientInsights exports exist. Verify all 7 functions are properly cached and use requireAdmin().</verify>
  <done>All 7 communication insights query functions created: getTopicOverview, getTopicTrend, getSentimentTrend, getChannelHotspots, getWeekOverWeek, getEscalationSummary, getClientInsights. All cached, admin-gated, org-scoped. getClientInsights returns empty array if no client profiles exist.</done>
</task>
...
2. communication-insights.ts exports 7 functions
...
8. getClientInsights returns empty array when no client profiles exist for the org
9. getClientInsights joins topicClassifications -> clientContacts -> clientProfiles correctly
</verification>

<success_criteria>
Complete query library for communication insights dashboard. Seven cached, admin-gated functions providing topic overview, topic trends, sentiment trends, channel hotspots, week-over-week comparison, escalation summary, and client-specific insights. getClientInsights joins through client_contacts to client_profiles and gracefully returns empty when no client profiles are configured. All queries are org-scoped and follow the response-time-analytics.ts pattern.
</success_criteria>
