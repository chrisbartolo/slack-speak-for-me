...
must_haves:
  truths:
    ...
    - "Admin can see client-specific insights table when client profiles exist (conditional render)"
  ...
  artifacts:
    ...
    - path: "apps/web-portal/components/admin/communication-insights-charts.tsx"
      ...
      exports: ["TopicTrendChart", "SentimentTrendChart", "ChannelHotspotTable", "WeekOverWeekCards", "EscalationSummaryCard", "ClientInsightsTable"]
  key_links:
    - from: "apps/web-portal/app/admin/communication-insights/page.tsx"
      to: "apps/web-portal/lib/admin/communication-insights.ts"
      via: "imports and calls all 7 query functions"
      pattern: "getTopicOverview|getTopicTrend|getSentimentTrend|getChannelHotspots|getWeekOverWeek|getEscalationSummary|getClientInsights"
    - from: "apps/web-portal/app/admin/communication-insights/page.tsx"
...
7. `ClientInsightsTable` component:
   - Props: `{ clients: ClientInsight[] }`
   - Only renders content when clients array is non-empty (the parent page conditionally renders this component, but the component itself also handles the empty case by returning null)
   - Renders an HTML table inside a Card with heading "Client Communication Patterns" and description "Topic and sentiment breakdown per client (based on client profiles)"
   - Table columns:
     - Client Name: text, left-aligned
     - Top Topic: text, the most frequent topic for that client
     - Complaint Rate (%): formatted to 1 decimal with % sign, color-coded (>20% red, >10% orange, else green)
     - Escalations: integer count, bold red if > 0
   - Rows sorted by complaint rate descending (highest risk clients first)
   - If clients array is empty, return null (component not rendered)
  </action>
  <verify>Run `npx tsc --noEmit` in apps/web-portal. Verify all 6 chart components are exported.</verify>
  <done>Six chart components created: TopicTrendChart (stacked area), SentimentTrendChart (line), ChannelHotspotTable (table), WeekOverWeekCards (comparison grid), EscalationSummaryCard (summary), ClientInsightsTable (conditional client table). All use Tremor and Tailwind.</done>
</task>
...
```typescript
let topicOverview, topicTrend, sentimentTrend, hotspots, weekComparison, escalations, clientInsights;
try {
  [topicOverview, topicTrend, sentimentTrend, hotspots, weekComparison, escalations, clientInsights] = await Promise.all([
...
<div className="p-6 space-y-6">
  ...
  {/* Client Insights — only rendered if client profiles exist */}
...
- All data fetched in parallel with Promise.all (including clientInsights)
...
- ClientInsightsTable conditionally rendered only when clientInsights array is non-empty (getClientInsights returns [] when no client profiles exist)
  </action>
  ...
  <done>Communication insights dashboard page created at /admin/communication-insights with topic overview cards, stacked area chart for topics, line chart for sentiment, hotspot table, week-over-week comparison, escalation summary, and conditional client insights table. All data fetched server-side with proper error handling. Client insights section only shown when client profiles exist.</done>
</task>
...
10. ClientInsightsTable renders when client profiles exist, hidden otherwise
11. ClientInsightsTable shows client name, top topic, complaint rate, escalation count
</verification>

<success_criteria>
Complete communication insights dashboard at /admin/communication-insights with 6 visualization components. Admin can view topic distribution trends, sentiment patterns, channel hotspots, week-over-week comparisons, escalation summaries, and client-specific communication pattern insights. Client insights section conditionally rendered only when the organization has client profiles configured. All data loaded server-side with React cache() and proper error handling.
</success_criteria>
