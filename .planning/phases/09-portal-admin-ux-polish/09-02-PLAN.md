...
<task type="auto">
  ...
  <action>
Update the NavItem component to support a `compact` prop for nested navigation items:

1. Add `compact?: boolean` to the NavItemProps interface

2. Update the Link className to conditionally adjust padding when compact:
   - Normal: `px-4 py-2.5`
   - Compact: `px-3 py-2` (smaller padding for nested items)

3. Also make the icon prop optional (compact items in admin submenu don't need icons):
   - Change interface to `icon?: LucideIcon`
   - Conditionally render the icon only if provided

Updated component should look like:
```tsx
