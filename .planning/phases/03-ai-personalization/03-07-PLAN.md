...
<task type="auto">
  <name>Task 2: Update callers to provide workspaceId and userId</name>
  <files>apps/slack-backend/src/workers/ai-processor.ts</files>
  <action>
The AI processor worker that calls generateSuggestion needs to be updated to pass workspaceId and userId. Check the existing implementation and update:

1. Read the current ai-processor.ts
2. Ensure the job data includes workspaceId and userId
3. Pass these to generateSuggestion

If the job data already has workspaceId and userId (likely from event context), just wire them through. If not, the job enqueuing in event handlers needs updating (but this should already be available since it's used for token lookup).
  </action>
  <verify>
Run `npx tsc --noEmit -p apps/slack-backend/tsconfig.json` - check that ai-processor.ts compiles without type errors.
  </verify>
  <done>
AI processor passes workspaceId and userId to generateSuggestion.
  </done>
</task>
