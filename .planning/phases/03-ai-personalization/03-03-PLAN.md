...
must_haves:
  truths:
    - "System records refinement events when user modifies a suggestion"
    - "System extracts patterns from refinement history (frequent edits, tone shifts)"
    - "Feedback patterns can be retrieved by userId"
  artifacts:
    - path: "apps/slack-backend/src/services/personalization/feedbackTracker.ts"
      provides: "Refinement tracking and pattern extraction"
      exports: ["trackRefinement", "getRefinementPatterns", "RefinementEvent"]
  key_links:
    - from: "apps/slack-backend/src/services/personalization/feedbackTracker.ts"
      to: "packages/database"
      via: "drizzle client for refinementFeedback table"
      pattern: "import.*from.*@slack-speak/database"
---

<objective>
Create the refinement feedback tracking service that records how users modify AI suggestions and extracts patterns to improve future suggestions.

Purpose: AI-07 and AI-08 require tracking refinement modifications and using them to improve suggestions. This service stores refinement events and provides pattern extraction for the style context builder.

Output: feedbackTracker.ts with trackRefinement() to record events and getRefinementPatterns() to extract learned patterns like frequent edits and tone adjustments.
</objective>
...
<task type="auto">
  <name>Task 1: Create feedback tracker service</name>
  <files>apps/slack-backend/src/services/personalization/feedbackTracker.ts</files>
  <action>
Create feedbackTracker.ts with:

```typescript
import { eq, and, gte, desc } from 'drizzle-orm';
import { db } from '@slack-speak/database';
import { refinementFeedback } from '@slack-speak/database/schema';
...
Run `npx tsc --noEmit -p apps/slack-backend/tsconfig.json` - should compile without errors.
  ...
  <done>
feedbackTracker.ts exports trackRefinement, getRefinementPatterns, and RefinementEvent type.
  </done>
</task>

<task type="auto">
  <name>Task 2: Export feedback tracker from personalization index</name>
  <files>apps/slack-backend/src/services/personalization/index.ts</files>
  <action>
Update the personalization index.ts to include feedback tracker exports:
...
Run `npx tsc --noEmit -p apps/slack-backend/tsconfig.json` to verify exports work.
  ...
  <done>
feedbackTracker functions are accessible via services/personalization.
  </done>
</task>
...
2. trackRefinement accepts RefinementEvent and stores to database
3. getRefinementPatterns returns frequentAdditions, frequentRemovals, toneShift, lengthTrend
4. detectRefinementType correctly identifies tone/length/word_choice/structure changes
</verification>

<success_criteria>
- trackRefinement() stores refinement events to database with auto-detected type
- getRefinementPatterns() analyzes 30-day history and returns patterns with 2+ occurrence threshold
- Pattern extraction identifies frequent additions/removals, tone shifts, and length trends
- All exports compile and are accessible
</success_criteria>

<output>
After completion, create `.planning/phases/03-ai-personalization/03-03-SUMMARY.md`
</output>
